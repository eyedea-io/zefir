...
export default function connect(ComposedComponent) {
  if (ComposedComponent.connectedComponent) {
    console.error(
      `Component ${ComposedComponent.connectedComponent} is already connected.`
    )
  }
  ...
  if (WrappedComponent.form) {
    WrappedComponent = formize(WrappedComponent.form)(
      WrappedComponent,
